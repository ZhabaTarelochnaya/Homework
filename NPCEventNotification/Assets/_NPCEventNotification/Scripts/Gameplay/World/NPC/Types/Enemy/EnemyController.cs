...
namespace NPCEventNotification.Scripts.Gameplay.NPC
{
    public class EnemyController : MonoBehaviour, IHealthUser
    {
        ...
        void Awake()
        {
            ...
            if (!_npcDetector) Debug.Log($"{gameObject.name}: _npcDetector is not set");
            if (!_enemyDataSo) Debug.Log($"{gameObject.name}: _enemyDataSo is not set");
            if (!_attackZone) Debug.Log($"{gameObject.name}: _attackZone is not set");
